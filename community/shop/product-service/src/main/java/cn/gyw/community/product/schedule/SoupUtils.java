...
public class SoupUtils {
			...
			try {
				...
				if (statusCode == 200) {
					...
					try {
						...
						// String id =
						// item.select("div[class='tb-detail-hd']").select("h1").attr("data-spm");
						...
						// String priceStr =
						// item.select("div[class='tm-price-panel']").select("div[class='tm-promo-type']").select("span[class='tm-price']").text();
	...
	public static Product soupTaobaoDetailById(Long number) {
	...
	// 天猫搜索功能
	public static List<Product> soupTmallByKeyWord(String keyword) {
		...
		try {
			...
			try {
				...
				if (statusCode == 200) {
					...
					Elements liList = ulList.select("div[class='product']");
					productList = new ArrayList<>(liList.size());
					// 循环liList的数据（具体获取的数据值还得看doc的页面源代码来获取，可能稍有变动）
					for (Element item : liList) {
						Product product = new Product();
						// 商品ID
						String id = item.select("div[class='product']").select("p[class='productStatus']")
								.select("span[class='ww-light ww-small m_wangwang J_WangWang']").attr("data-item");
						product.setNumber(Long.parseLong(id));
						// 商品名称
						...
						product.setUrl(goodsUrl);
						// 商品图片网址
						String imgUrl = item.select("div[class='productImg-wrap']").select("a").select("img")
