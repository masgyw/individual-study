...
import cn.gyw.community.security.filter.JwtAuthenticationFilter;
import cn.gyw.community.security.filter.JwtLoginFilter;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	 ...
	 * 开放访问的请求
	 */
	private final static String[] PERMIT_ALL_MAPPING = { "/api/hello", "/api/login", "/api/home", "/api/verifyImage",
			"/api/image/verify", "/images/**" };
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(PERMIT_ALL_MAPPING).permitAll()
				.antMatchers("/api/user/**", "/api/data", "/api/logout")
				// USER 和 ADMIN 都可以访问
				.hasAnyAuthority(USER, ADMIN).antMatchers("/api/admin/**")
				// 只有 ADMIN 才可以访问
				...
				// 登陆过滤器
				.addFilterBefore(
						new JwtLoginFilter("/s/login", authenticationManager(), verifyCodeService, loginCountService),
