...
import cn.gyw.frame.thirdpart.media.domain.AudioMetaInfo;
import cn.gyw.frame.thirdpart.media.domain.ImageMetaInfo;
import cn.gyw.frame.thirdpart.media.domain.VideoMetaInfo;
 ...
 * 音视频工具类
 *
 * @desc 基于FFmpeg内核来编解码音视频信息；
 * 1）使用前需手动在运行环境中安装FFmpeg运行程序
 * 2）正确设置FFmpeg运行路径
 * 3）MediaUtil.java正常调用到FFmpeg程序去处理音视频
...
public class MediaUtil {
    ...
    private static Time DEFAULT_TIME = new Time(0, 0, 10);
    /**
     * 视频帧抽取的默认宽度值，单位：px
     */
     ...
     * 用于解析音频详细信息时使用；
     */
    private static String audioStreamRegex = "Stream #\\d:\\d[\\(]??\\S*[\\)]??: Audio: (\\S*\\S$?)(.*), (.*?) Hz, (.*?), (.*?), (\\d*) kb\\/s";
    private static Pattern audioStreamPattern;

    /**
     * 静态初始化时先加载好用于音视频解析的正则匹配式
     *
     * 如果你需要解析其他后缀格式如 .MKV .MP3 这样的媒体文件时，
     * 需要根据实际情况修改durationRegex videoStreamRegex musicStreamRegex 这三个正则表达式的值，
     * 否则可能无法解析出正确的信息
     */
    static {
        durationPattern = Pattern.compile(durationRegex);
        videoStreamPattern = Pattern.compile(videoStreamRegex);
        audioStreamPattern = Pattern.compile(audioStreamRegex);
    }
    ...
    public static void convertVideo(File fileInput, File fileOutPut, boolean withAudio, Integer crf, String preset, Integer width, Integer height) {
        ...
        command.add(crf.toString()); // 视频质量参数，值越小视频质量越高
    ...
    private static void cutVideoFrame(File videoFile, String path, Time time, int width, int height, int timeLength, boolean isContinuty) {
        ...
        try {
            ...
            if (isContinuty) {
                command.add(path + File.separator + "foo-%03d.jpeg");
            } else {
                command.add(path);
            }
    ...
    public static void getAudioFromVideo(File videoFile, File audioFile) {
        ...
        try {
            ...
            command.add("-i");
            command.add("\"" + videoFile.getAbsolutePath() + "\"");
            command.add("-vn"); // no video，去除视频信息
            command.add("-y");
            command.add("-acodec");
            command.add("copy");
    ...
    public static VideoMetaInfo getVideoMetaInfo(File videoFile) {
        ...
        Matcher videoMusicStreamMacher = audioStreamPattern.matcher(parseResult);
        ...
        AudioMetaInfo.setFormat(musicFormat);
        AudioMetaInfo.setDuration(duration);
        AudioMetaInfo.setBitRate(musicBitrate);
        AudioMetaInfo.setSampleRate(samplerate);
        // 封装视频信息
        ...
        videoMetaInfo.setAudioMetaInfo(AudioMetaInfo);
     ...
     */
    public static AudioMetaInfo getAudioMetaInfo(File musicFile) {
        ...
        Matcher musicStreamMatcher = audioStreamPattern.matcher(parseResult);
        ...
        try {
            ...
            if (durationMatcher.find()) {
                ...
                long dec = (long) Integer.parseInt(durationMatcher.group(4));
                duration = dec * 100L + seconds * 1000L + minutes * 60L * 1000L + hours * 60L * 60L * 1000L;
                //String startTime = durationMacher.group(5) + "ms";
        ...
        AudioMetaInfo.setSampleRate(samplerate);
        // 音频大小
        AudioMetaInfo.setSize(musicFile.length());
    ...
    public static AudioMetaInfo getAudioMetaInfo(InputStream inputStream) {
        ...
        try {
            ...
            FileUtils.copyInputStreamToFile(inputStream, file);
            AudioMetaInfo = getAudioMetaInfo(file);
            file.deleteOnExit();
            return AudioMetaInfo;
        } catch (Exception e) {
    ...
    public static boolean isGivenFormat(File inputFile, String[] givenFormat) {
        if (null == inputFile || !inputFile.exists()) {
            log.warn("无法检查文件类型是否满足要求，因为要检查的文件不存在 源文件： " + inputFile);
            return false;
        }
        if (null == givenFormat || givenFormat.length <= 0) {
            log.warn("无法检查文件类型是否满足要求，因为没有指定的文件类型");
            return false;
        }
        String format = getFormat(inputFile);
    ...
    public static String getMetaInfoFromFFmpeg(File inputFile) {
        ...
        return MediaUtil.executeCommand(command);
    }
    ...
    public static boolean isExecutable() {
        ...
        cmds.add("-version");
        String ffmpegVersionInfo = executeCommand(cmds);
        return !StringUtils.isBlank(ffmpegVersionInfo);
    }
    ...
    public static String executeCommand(List<String> commands) {
        if (CollectionUtils.isEmpty(commands)) {
            log.warn("指令执行失败，因为要执行的FFmpeg指令为空！");
            return null;
        }
        ...
        try {
            ...
            // 如果不取的话当输出流信息填满jvm存储输出留信息的缓冲区时，线程会阻塞住
            ...
            String runRet = StringUtils.isBlank(result) ? "失败" : "成功";
            log.info("已执行FFmepg命令：{},执行结果：{}", cmdStr, runRet);
            return result;
        } catch (Exception e) {
     ...
     *
     * @param ffmpegPath ffmpeg可执行程序在实际系统中的绝对路径
     * @return
     */
    public static boolean setFFmpegPath(String ffmpegPath) {
        if (StringUtils.isBlank(ffmpegPath)) {
            log.warn("设置ffmpeg执行路径失败，因为传入的ffmpeg可执行程序路径为空！");
            return false;
        }
        File ffmpegFile = new File(ffmpegPath);
        if (!ffmpegFile.exists()) {
            log.warn("设置ffmpeg执行路径失败，因为传入的ffmpeg可执行程序路径下的ffmpeg文件不存在！");
            return false;
        }
        FFMPEG_PATH = ffmpegPath;
        log.warn("设置ffmpeg执行路径成功 当前ffmpeg可执行程序路径为： " + ffmpegPath);
        return true;
    }
    ...
    private static void createGifImage(String[] image, String outputPath, int playTime) {
        ...
        try {
            ...
            log.warn("多张静态图转换成动态GIF图的过程出错 错误信息： " + e.getMessage());
        }
    }
    ...
    private static class ProcessKiller extends Thread {
        ...
        public void run() {
            ...
            log.warn("已销毁FFmpeg进程 进程名： " + process.toString());
        }
    }
    ...
    static class PrintStream extends Thread {
        ...
        public void run() {
            try {
                ...
                log.warn("读取输入流出错了！错误信息：" + e.getMessage());
            } finally {
                try {
                    ...
                    log.warn("调用PrintStream读取输出流后，关闭流时出错！---");
                }
            }
        }
    }
