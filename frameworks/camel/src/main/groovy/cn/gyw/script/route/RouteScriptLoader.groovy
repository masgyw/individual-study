...
//            startWatcher()
    ...
    /*def loadAllConfigurations() {
        if (!new File(configurationPathUri).exists()) {
            logger.warn('route configuration path [{}] does not exist.  skip loading configurations', configurationPath)
            return
        }

        new File(configurationPathUri).traverse(type: FileType.FILES, nameFilter: ~/.*\.route/) { routeFile -> loadRoutes(routeFile) }
    }*/
    ...
    /*
     * watch the files change under root of route scripts
     *
     * CAUTION: Currently, only the root folder is watched.  Watch subfolders change will be implemented later.
