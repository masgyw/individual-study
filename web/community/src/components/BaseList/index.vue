    ...
    <ul>
      <li v-for="item in list" :key="item.product_id">
        ...
        <router-link :to="{ path: '/shop/goods/details', query: {productID:item.productId} }">
          <img :src="target +item.productPicture" :alt="target +item.productPicture" />
...
export default {
  ...
  data() {
    return {
      target: this.$store.getters.productHost,
    };
  },
  computed: {
...
.myList ul li img {
  ...
  width: 160px;
  height: 160px;
  background: url(../../assets/imgs/logo.png) no-repeat 50%;
  margin: 0 auto;
}
.myList ul li h2 {
  margin: 25px 10px 0;
