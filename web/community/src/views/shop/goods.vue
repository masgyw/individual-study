<template>
  <div class="goods" id="goods" name="goods">
    ...
    <div class="main">
      <div class="list">
        <BaseList :list="product" v-if="product.length>0"></BaseList>
...
export default {
  ...
  methods: {
    ...
    // 向后端请求分类列表数据
    getCategory() {
      this.BASE_APIS.findAll(apiTypes.CATEGORY)
        .then(resp => {
          const val = {
            categoryId: 0,
            categoryName: "全部"
          };
          const cate = resp.data;
          cate.unshift(val);
    ...
    getData() {
      ...
      this.$axios
        ...
        this.BASE_APIS.findByPage(apiTypes.PRODUCT_INFO, {
          categoryID: this.categoryID,
          page: this.currentPage,
          limit: this.pageSize
        })
        .then(resp => {
          this.product = resp.data.records;
