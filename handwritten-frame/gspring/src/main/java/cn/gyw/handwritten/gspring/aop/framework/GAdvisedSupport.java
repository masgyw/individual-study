...
import cn.gyw.handwritten.gspring.aop.aspect.GAfterReturningAdviceInterceptor;
import cn.gyw.handwritten.gspring.aop.aspect.GAfterThrowingAdviceInterceptor;
import cn.gyw.handwritten.gspring.aop.aspect.GMethodBeforeAdviceInterceptor;
...
public class GAdvisedSupport {
            ...
            for (Method m : this.targetClass.getMethods()) {
                ...
                if (matcher.matches()) {
                    ...
                    if (!(null == config.getAspectAfter() || "".equals(config.getAspectAfter().trim()))) {
                        // 创建一个通知对象
						advices.add(new GAfterReturningAdviceInterceptor(aspectMethods.get(this.config.getAspectAfter()),
								aspectClass.newInstance()));
                    }
                    ...
                    if (!(null == config.getAspectBefore() || "".equals(config.getAspectBefore().trim()))) {
                        // 创建一个通知对象
						GAfterThrowingAdviceInterceptor afterThrowingAdvice =
								new GAfterThrowingAdviceInterceptor(aspectMethods.get(this.config.getAspectAfterThrow()),
								aspectClass.newInstance());
    ...
    public boolean pointCutMatch() {
        this.pointCutClassPattern.matcher(this.targetClass.toString()).matches();
        return true;
    }
